...
import { generateUniqueSelector } from './utils/domUtils';
...
const DraggablePin: React.FC<PinProps> = ({
  ...
  const pinPosition = useMemo(() => {
    ...
    try {
      ...
      // 타겟 요소 크기 기준 퍼센트로 위치 계산
      const absoluteLeft = rect.left + scrollX + (rect.width * (spec.percentX || 0) / 100);
      const absoluteTop = rect.top + scrollY + (rect.height * (spec.percentY || 0) / 100);
      
      return { top: absoluteTop, left: absoluteLeft };
    } catch {
      return { top: 0, left: 0 };
    }
  }, [iframeDoc, spec.targetSelector, spec.percentX, spec.percentY, domVersion]);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const handleMouseUp = () => {
      ...
      // Read final position from DOM and convert to percent
      if (pin && iframeDocRef) {
        ...
        try {
          ...
          if (targetElement) {
            ...
            // 핀의 타겟 내 상대 위치
            const relativeX = finalLeft - (rect.left + scrollX);
            const relativeY = finalTop - (rect.top + scrollY);
            
            // 퍼센트로 변환
            const newPercentX = rect.width > 0 ? (relativeX / rect.width) * 100 : 50;
            const newPercentY = rect.height > 0 ? (relativeY / rect.height) * 100 : 50;
            
            onDragEnd(spec.id, newPercentX, newPercentY);
          }
        } catch {
          // 실패 시 무시
        }
...
const App: React.FC = () => {
  ...
  const [iframeContentWidth, setIframeContentWidth] = useState<number | null>(null);
  ...
  const handleAddSpecRef = useRef<(data: { id: string; targetSelector: string; percentX: number; percentY: number }) => void>(() => { });

  // Keep panel compact on narrower screens so the viewer gets more space
  useEffect(() => {
    const updatePanelLayout = () => {
      setIsCompactPanel(window.innerWidth < 1400);
    };
    updatePanelLayout();
  ...
  // Track iframe content width to prefer horizontal scroll over forced scaling
  const updateIframeContentSize = useCallback(() => {
    const doc = iframeRef.current?.contentDocument;
    const win = iframeRef.current?.contentWindow;
    if (!doc || !win) return;

    const scrollWidth = doc.documentElement?.scrollWidth || 0;
    const bodyScrollWidth = doc.body?.scrollWidth || 0;
    const clientWidth = doc.documentElement?.clientWidth || 0;
    const innerWidth = win.innerWidth || 0;

    const measuredWidth = Math.max(scrollWidth, bodyScrollWidth, clientWidth, innerWidth);
    setIframeContentWidth(prev => (prev === measuredWidth ? prev : measuredWidth));
  }, []);

  // --- Spec CRUD Handlers ---
  const handleAddSpec = useCallback((data: { id: string; targetSelector: string; percentX: number; percentY: number }) => {
  ...
  // Pin drag end handler - 퍼센트로 저장
  const handlePinDragEnd = (id: string, newPercentX: number, newPercentY: number) => {
    setSpecs(prev => prev.map(s =>
      s.id === id ? { ...s, percentX: newPercentX, percentY: newPercentY } : s
    ));
    handleSelectSpec(id);
  };
  ...
  const attachDocListeners = useCallback((doc: Document) => {
    ...
    const clickHandler = (e: MouseEvent) => {
      if (e.ctrlKey || e.metaKey) {
        ...
        // 클릭한 정확한 요소를 타겟으로 사용 (findStableContainer 제거)
        const targetElement = clickedElement;
        
        // 타겟 요소 기준 퍼센트 계산
        const targetRect = targetElement.getBoundingClientRect();
        const clickXInTarget = e.clientX - targetRect.left;
        const clickYInTarget = e.clientY - targetRect.top;
        
        // 퍼센트로 변환 (요소 크기가 변해도 같은 상대 위치 유지)
        const percentX = targetRect.width > 0 ? (clickXInTarget / targetRect.width) * 100 : 50;
        const percentY = targetRect.height > 0 ? (clickYInTarget / targetRect.height) * 100 : 50;

        const selector = generateUniqueSelector(targetElement);
  ...
  const handleIframeLoad = useCallback(() => {
    ...
    if (doc) {
      ...
      // Measure initial iframe content width to avoid responsive shifts on smaller screens
      updateIframeContentSize();
    }
  }, [attachDocListeners, updateIframeContentSize]);
  ...
  }, [htmlContent, updateIframeContentSize]);
  ...
  }, [iframeDoc, updateIframeContentSize]);

  // Keep iframe content width in sync when DOM mutations change layout
  useEffect(() => {
    if (!iframeDoc) return;
    updateIframeContentSize();
  }, [domVersion, iframeDoc, updateIframeContentSize]);
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {/* Iframe / Drop zone */}
          <div className="relative flex-1 overflow-x-auto overflow-y-hidden bg-white">
            ...
            {htmlContent && (
              <>
                <div
                  ...
                  style={{ minWidth: iframeContentWidth ? `${iframeContentWidth}px` : '100%' }}
                >
                  <iframe
                    ref={iframeRef}
                    srcDoc={htmlContent}
                    ...
                    style={{ width: iframeContentWidth ? `${iframeContentWidth}px` : '100%' }}
        ...
        <div className={`${isCompactPanel ? 'w-[260px] min-w-[220px] max-w-[300px]' : 'w-[340px] min-w-[260px] max-w-[420px]'} flex-shrink-0 border-l bg-white overflow-hidden flex flex-col`}>
          <EditorPanel
            specs={filteredSpecs}
