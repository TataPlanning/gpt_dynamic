/**
 * Export Utilities
 * HTML Export with Responsive Pin Positioning (Anchor-based Fixed Offset)
 */
...
/**
 * Export HTML을 생성합니다.
 * 타겟 요소 좌상단 기준 + 고정 오프셋(px) + resize 이벤트 대응
 * 
 * @param originalHtml - 원본 HTML 문자열
 * @param specs - Spec 아이템 배열
 * @param iframeDoc - iframe의 Document 객체 (현재 미사용, 향후 확장용)
...
export const generateExportHTML = (
  ...
  // Export용 spec 데이터 (offsetX, offsetY 사용)
...
(function() {
  ...
  /**
   * 모든 핀을 렌더링합니다.
   * 타겟 요소 좌상단 기준 + 고정 오프셋(px)으로 위치 계산
   * 화면 크기가 변해도 오프셋은 고정 → 반응형 대응
   */
  function renderPins() {
    // 기존 핀 모두 제거
    ...
    const visibleSpecs = specs
      .filter(spec => {
        // pageId가 없으면 모든 페이지에 표시
        if (!spec.pageId) return true;
        // pageId가 있으면 현재 페이지와 일치해야 표시
    ...
    visibleSpecs.forEach((spec, index) => {
      ...
      // 타겟을 못 찾으면 핀 표시 안 함 (Ghost Pin 방지)
      if (!targetElement) {
      ...
      // ★ 핵심: 타겟 요소 좌상단 기준 + 고정 오프셋(px)
      const rect = targetElement.getBoundingClientRect();
      const scrollX = window.scrollX || window.pageXOffset;
      const scrollY = window.scrollY || window.pageYOffset;
      
      // 타겟 좌상단 절대 위치 + 고정 오프셋
      const pinLeft = rect.left + scrollX + spec.offsetX;
      const pinTop = rect.top + scrollY + spec.offsetY;
  ...
  function init() {
    ...
    window.addEventListener('resize', () => {
      ...
      resizeTimer = setTimeout(renderPins, 10); // 0.01초 debounce
    });
